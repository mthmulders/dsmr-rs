        ...
        Some(excl_mark) => {
            ...
            match buffer[excl_mark..].find('\n') {
                Some(line_end) => {
                    ...
                    log::trace!("Exclamation mark found at index {}, returning index {}", excl_mark, end);
                    Some(end)
                }
                None => {
                    log::trace!("Exclamation mark found at index {}, but lines doesn't end", excl_mark);
                    None
                }
            }
        }
...
fn eat_telegrams<'a>(buffer: &'a str, consumer: &mut dyn TelegramConsumer) -> Cow<'a, str> {
    ...
    match (start_index, end_index) {
        (Some(start), Some(end)) => {
            ...
            log::trace!("Start and end found, consuming complete telegram from index {} to {}:\n{}", start, end, telegram);
        ...
        (None, None) => {
            log::trace!("There is no start and no end of the telegram, keeping buffer {}", buffer);
            Cow::Borrowed(buffer)
        }     
    }
}
...
mod tests {
    ...
    fn find_end_of_telegram() {
        ...
        assert_eq!(super::find_end_of_telegram("abcd\r\n!522B\r\n", 0), Some(12));
        assert_eq!(super::find_end_of_telegram("abcd\r\n!\r\n", 0), Some(8));
        assert_eq!(super::find_end_of_telegram("!522B\r\n\r\nabcd\r\n\r\n", 10), None);
    }
    ...
    impl TestConsumer {
        fn new() -> Self {
            TestConsumer { invoked: false, telegram: String::new() }
        }
    }
    ...
    fn eat_telegrams_start_and_end() {
        let mut text = String::from("/ISk5\\2MT382-1000\r\n\r\n1-3:0.2.8(40)\r\n!522B\r\n\r\n/ISk5\\2MT382-1000");
        let mut consumer = TestConsumer::new();

        let result = eat_telegrams(&mut text, &mut consumer);

        assert_eq!(consumer.invoked, true);
        assert_eq!(consumer.telegram, "/ISk5\\2MT382-1000\r\n\r\n1-3:0.2.8(40)\r\n!522B\r\n");
        assert_eq!(result, "\r\n/ISk5\\2MT382-1000");
    }
}